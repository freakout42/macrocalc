/* $Id: sql_crep.c,v 1.2 2020/05/31 11:36:24 axel Exp $
 * create PgSql-table from dbf-descriptor
 */
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
for (fld=1; fld<=d->fields; fld++)
	...
	*f = *fields++;
	...
	switch (f->typ)
	 {
	 case 'D': fprintf (d->ofile_ptr, "date"); break;
	 case 'N': fprintf (d->ofile_ptr, f->dec==0 ? "integer" : "numeric(%d,%d)", f->len, f->dec); break;
	 case 'F': fprintf (d->ofile_ptr, "numeric(%d,%d)", f->len, f->dec); break;
	 case 'L': fprintf (d->ofile_ptr, "integer"); break;
	 case 'C': fprintf (d->ofile_ptr, "varchar(%d)", f->len); break;
	 case 'M':
